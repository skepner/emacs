...
(defun eu-compile-cd-pwd ()
  ...
  (let* ((dir (directory-file-name (file-name-directory (buffer-file-name))))
         ...
         (cmd (cond
               ...
               ;((and (string-match "Makefile" basename) (string-match "/AD/" dirname)) (concat "gtime gmake -C " dirname " -w -j8 -k --output-sync=line T=R"))
               ((and (string-match "Makefile" basename) (string-match "/AD/" dirname)) (concat "gtime gmake -C " dirname " -j8 -k"))
               ((string-match "Makefile" basename) (concat "gmake -w --output-sync -C " dirname " -j8 -k"))
               ((string-match "CMakeLists.txt" basename) (concat "gmake -w --output-sync -C " dirname "/build -j8 -k"))
